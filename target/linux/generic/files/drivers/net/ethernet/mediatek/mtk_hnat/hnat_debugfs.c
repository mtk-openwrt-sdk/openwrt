...
int binding_threshold(int threshold)
{
	...
	writel(threshold, host->ppe_base + PPE_BNDR);
	return 0;
}
...
int tcp_bind_lifetime(int tcp_life)
{
	...
	cr_set_field(host->ppe_base + PPE_BND_AGE_1, TCP_DLTA, tcp_life);
...
int fin_bind_lifetime(int fin_life)
{
	...
	cr_set_field(host->ppe_base + PPE_BND_AGE_1, FIN_DLTA, fin_life);
...
int udp_bind_lifetime(int udp_life)
{
	...
	cr_set_field(host->ppe_base + PPE_BND_AGE_0, UDP_DLTA, udp_life);
...
int tcp_keep_alive(int tcp_interval)
{
	...
	cr_set_field(host->ppe_base + PPE_KA, TCP_KA, tcp_interval);
...
int udp_keep_alive(int udp_interval)
{
	...
	cr_set_field(host->ppe_base + PPE_KA, UDP_KA, udp_interval);
...
static struct hnat_accounting *hnat_get_count(struct hnat_priv *h, u32 index)
{
...
static int hnat_debug_show(struct seq_file *m, void *private)
{
	...
	end = h->foe_table_cpu + foe_etry_num;
	while (entry < end) {
			       ...
			       size_t count, loff_t *ppos)
{
	...
	qtx_cfg = readl(h->fe_base + QTX_CFG(id % NUM_OF_Q_PER_PAGE));
	qtx_sch = readl(h->fe_base + QTX_SCH(id % NUM_OF_Q_PER_PAGE));
	len += scnprintf(buf + len, buf_len - len, "packet count: %u\n", qtx_cfg);
	len += scnprintf(buf + len, buf_len - len, "packet drop: %u\n\n", qtx_sch);

	/* Recover to normal mode */
	cr_set_field(host->fe_base + QTX_MIB_IF, MIB_ON_QTX_CFG, 0);
	cr_set_field(host->fe_base + QTX_MIB_IF, VQTX_MIB_EN, 0);
...
int get_ppe_mib(int index, u64 *pkt_cnt, u64 *byte_cnt)
{
	...
	entry = host->foe_table_cpu + index;
...
int __init hnat_init_debugfs(struct hnat_priv *h)
{
	...
	for (i = 0; i < host->data->num_of_sch; i++) {
