...
static int mcast_entry_get(u16 vlan_id, u32 dst_mac)
{
	int index = -1;
	u8 i;
	struct ppe_mcast_group *p = host->pmcast->mtbl;
	u8 max = host->pmcast->max_entry;
...
static int set_hnat_mtbl(struct ppe_mcast_group *group, int index)
{
	...
	if (index < 0x10) {
		reg = host->ppe_base + PPE_MCAST_H_0 + ((index) * 8);
		writel(mcast_h.u.value, reg);
		reg = host->ppe_base + PPE_MCAST_L_0 + ((index) * 8);
		writel(mcast_l.addr, reg);
	} else {
		index = index - 0x10;
		reg = host->fe_base + PPE_MCAST_H_10 + ((index) * 8);
		writel(mcast_h.u.value, reg);
		reg = host->fe_base + PPE_MCAST_L_10 + ((index) * 8);
		writel(mcast_h.u.value, reg);
	}
...
{
	struct ppe_mcast_table *pmcast = host->pmcast;
...
int hnat_mcast_enable(void)
{
	...
	cr_set_field(host->ppe_base + PPE_GLO_CFG, MCAST_TB_EN, 1);
	/* multicast port0 map to PDMA */
	cr_set_field(host->ppe_base + PPE_MCAST_PPSE, MC_P0_PPSE, 0);
	/* multicast port1 map to GMAC1 */
	cr_set_field(host->ppe_base + PPE_MCAST_PPSE, MC_P1_PPSE, 1);
	/* multicast port2 map to GMAC2 */
	cr_set_field(host->ppe_base + PPE_MCAST_PPSE, MC_P2_PPSE, 2);
	/* multicast port3 map to QDMA */
	cr_set_field(host->ppe_base + PPE_MCAST_PPSE, MC_P3_PPSE, 5);
