...
function mtkwifi.update_profile(path1, path2)
    local cfg1 = mtkwifi.load_profile(path1)
    local cfg2 = mtkwifi.load_profile(path2)

    if not cfg1 or not cfg2 or not next(cfg2) then return end
    for k,v in pairs(cfg2) do
        cfg1[k] = v
    end

    mtkwifi.save_profile(cfg1, path1)
end
